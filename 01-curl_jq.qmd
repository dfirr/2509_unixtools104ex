...
スタートメニューで「terminal」とタイプし、表示されるターミナルアプリを選択して、
Windowsターミナルを表示させてください。

![](images/start_terminal.png){fig-align="center" width=50%}

Windowsターミナルでは、デフォルトでWindows PowerShellが起動しています。
「∨」マークをクリックし、表示されるメニューから「Ubuntu」を選択してください。

![](images/terminal_ubuntu.png){fig-align="center" width=50%}

## curl
### 単純な閲覧 {.unnumbered}
阿部寛のホームページに、curlを使ってアクセスします。以下のコマンドを
入力してください。

::: {.callout-note}
コードブロックの右端にマウスカーソルを当てると📋アイコンが表示されます。
クリックすると、コードがコピーできます。
:::
...
JSON形式は入れ子構造をもち、行指向ではないので、古典的なUnixツール
（sed, awk, grepなど）で処理することは適切ではありません。
かわりにjqを使うことで、awkに匹敵する処理が可能になります。

jqはUbuntu 24.0.3にプリインストールされていないので、`apt`コマンドで
インストールしてください。
...
jqがインストールされているかどうかは、`which`コマンドで確かめられます。
...
- `cut`コマンドの`-c`オプションは、指定したバイトを切り出します。
`-c 3-`と指定することで、3バイト目以降を切り出しています。
（すなわち`> `を削除している。）
...
- `//`は代替演算子で、`.`の値が`null`や`false`であれば右辺（`0`）を返します。
...
::: {.callout-tip title="ヒント" collapse="true"}
並び替えには`sort`を使いますが、`sort`にわたるデータは
オブジェクトではなく配列でなければいけません。
...
- Daniel Stenberg「[curl man page](https://curl.se/docs/manpage.html)」
- Koichi Nakashima「[新しいcurlコマンドの使い方 完全ガイド（2025年版）](https://qiita.com/ko1nksm/items/30982a5f357f26ae166f)」
- jqlang「[jq 1.8 Manual](https://jqlang.org/manual/)」
